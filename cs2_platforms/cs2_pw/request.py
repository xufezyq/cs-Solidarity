...
logger = logging.getLogger("CS2PF")
...
class PerfectWorldApi:
    ...
    def __init__(self, uid: Optional[str] = None, token: Optional[str] = None):
        self.uid = uid
        self.token = token
    ...
    async def get_token(self) -> Optional[List[str]]:
        ...
        # 如果没有初始化提供，这里可以抛出异常或者返回None
        logger.warning("No UID or Token provided in initialization.")
        return None
        ...
        async with AsyncClient(verify=self.ssl_verify) as client:
            try:
                ...
                try:
                    ...
                    return raw_data
                if "result" in raw_data and isinstance(raw_data["result"], dict) and "error_code" in raw_data["result"]:
                    return raw_data["result"]["error_code"]
                elif isinstance(raw_data, dict) and raw_data.get("code") not in [0, 1] and raw_data.get("code") is not None:
    ...
    async def get_match_mvp(self, matchid: str):
        ...
        if isinstance(data, int):
            return data
        elif data.get("statusCode") != 0:
            return data.get("data")
        return cast(MatchTitel, data["data"])

    async def get_match_detail(self, matchid: str, dataSource: int = 3):
        """搜索对局详情信息"""
        uid_token = await self.get_token()
        if uid_token is None:
            return -1
