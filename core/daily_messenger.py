...
class DailyMessenger:
    ...
    def start(self, check_interval=60):
        """
        启动每日定时发送
        :param check_interval: 保留与统一接口一致（未使用）
        """
        print(f"[{datetime.now()}] DailyMessenger 启动，计划每天 {self.send_time} 发送固定消息")
        ...
        try:
            ...
            print(f"\n[{datetime.now()}] DailyMessenger 程序已停止")

    @staticmethod
    def create_from_config(config_path):
        """从配置文件创建 DailyMessenger 实例"""
    ...
    def create_from_data(data):
        """从字典数据创建 DailyMessenger 实例"""
        if not isinstance(data, dict):
            raise TypeError("DailyMessenger.create_from_data 需要传入字典数据")

        return DailyMessenger(
            wechat_groups=data.get('wechat_groups', ['文件传输助手']),
