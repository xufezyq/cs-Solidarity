...
class ChatServer(socketserver.ThreadingMixIn, socketserver.TCPServer):
    ...
    def process_message(self, msg):
        ...
        if action == "steam.reset_official_season_records":
            return self._handle_steam_reset_official_season_records()
    ...
    def _handle_steam_reset_official_season_records(self):
        """清空运行中 Steam 实例的官匹赛季历史统计。"""
        if not self._instances_ref:
            return {"success": False, "error": "Bot 尚未初始化实例"}

        with self._steam_reset_lock:
            for name, inst in self._instances_ref:
                reset_fn = getattr(inst, "reset_official_season_records", None)
                if callable(reset_fn):
                    data = reset_fn()
                    data["instance"] = name
                    return {"success": True, "data": data}
