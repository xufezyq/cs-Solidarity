...
@router.post("/steam/official-season/reset")
async def reset_steam_official_season_records(current_user: User = Depends(require_admin)):
    """管理员手动清空官匹赛季历史统计。"""
    result = await bridge.send_request("steam.reset_official_season_records", timeout=30)
    if not result.get("success"):
        raise HTTPException(status_code=502, detail=result.get("error", "Agent 请求失败"))
    return {"success": True, "data": result.get("data", {})}
