...
class AgentHandler:
    ...
    def _reset_5e_season_records_file(self) -> Dict[str, Any]:
        """Bot 未运行时，归档 Steam 数据文件中的 5E 赛季统计，然后清空。"""
        data_file = self._steam_data_file()
        data_file.parent.mkdir(parents=True, exist_ok=True)
        ...
        history = config.get("friend_5e_history_stats") or {}
    ...
    def _steam_reset_5e_season_records(self, params: Dict[str, Any]) -> Dict[str, Any]:
        """清空 5E 赛季历史极值。"""
        try:
            from bot.chat_server import send_action_to_bot
            result = send_action_to_bot("steam.reset_5e_season_records", {}, timeout=15)
            if result.get("success"):
                result.setdefault("data", {})["mode"] = "bot"
                return result
            if self._is_bot_process_running():
